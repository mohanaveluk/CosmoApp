...
DashboardApp.controller('DashboardController', [
    '$scope', '$http', '$interval', 'ReportService', function ($scope, $http, $interval, reportService) {
        ...
        $scope.getEnvironmentList = function() {
            ...
            try {
                $http.post('../forms/Generic.aspx/GetAllUrlConfiguration', dataObj)
                    .success(function(data, status, headers, config) {
                        $scope.environmentList = data.d;
                        if ($scope.environmentList != null && $scope.environmentList.length > 0) {
                            angular.forEach($scope.environmentList, function(entity) {

                                $scope.environments.push({ id: entity.EnvID });
                                if (entity.CreatedDateTime != null || entity.CreatedDateTime != undefined)
                                    entity.CreatedDateTime = reportService.parsedDate(entity.CreatedDateTime);
                                if (entity.UpdatedDateTime != null || entity.UpdatedDateTime != undefined)
                                    entity.UpdatedDateTime = reportService.parsedDate(entity.UpdatedDateTime);
                            });
                            $scope.handleError('');
                        } else {
                            $scope.handleError("No URL Configuration Data Exists");
        ...
        $scope.EditEnvironmentConfiguration = function (entity, type) {
            ...
            if (type === "c")
                openEditEnvironmentModal("UrlConfigurationDetails.aspx?s=" + entity.Id + "&t=ed");
            else if (type === "e")
                openEditEnvironmentModal("UrlConfigurationDetails.aspx?s=" + entity.EnvID + "&t=en");
            else if (type === "n") {
                openEditEnvironmentModal("UrlConfigurationDetails.aspx");
            }
        };
        ...
        $scope.DeleteEnvironmentConfiguration = function (entity) {
            var message = "Are you sure, you want to delete url configuration of " + entity.Adress + ", display: " + entity.DisplayName + "?";
            $scope.DeleteConfig(entity.Id, message, 'urlconfig');
        };
